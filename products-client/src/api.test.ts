...
describe('product API', () => {
  it('fetches products', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response(JSON.stringify([{ id: '1', name: 'P1', price: '10' , cost: '5'}]), { status: 200 })
    ));

    await expect(fetchProducts()).resolves.toEqual([{ id: '1', name: 'P1', price: '10', cost: '5'  }]);
    expect(fetch).toHaveBeenCalledWith('http://localhost:4000/api/products');
  });

  it('creates a product', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ id: '2', name: 'P2', price: '10.00', cost: '6.00' }), { status: 201 })
    ));
